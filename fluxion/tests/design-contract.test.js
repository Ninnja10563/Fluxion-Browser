...
test("Flow pointer closing compresses the vacated row without waiting for movement", () => {
...
test("Fluxion Library owns visible history, bookmark, and download workflows", () => {
  ...
  assert.match(library, /url === "about:blank" && tab\?\.hasAttribute\("fluxion-library-section"\)/);
  assert.match(library, /find\(isLibraryTab\)/);
...
test("Browser Memory exposes functional privacy controls", () => {
  ...
  assert.match(memory, /FluxionMemoryStore\.search\(query, 18, useEmbeddings, \{ onLexical: partial \}\)/);
