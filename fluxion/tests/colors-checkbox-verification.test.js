...
function fixture(palette) {
  const style = { appearance: "auto", renderer: "non-native", accentColor: rgb(colors.palette(palette).accent),
...
test("native checkbox gate checks Gecko renderer, visible checked control and derived accessible accent", () => {
  for (const palette of [{ base: "#202d36", accent: "#ff6bb5" }, { base: "#ddd7cd", accent: "#603d1c" },
    ...
    assert.equal(result.renderer, "non-native"); assert.equal(result.checked, true);
  }
});
test("native checkbox gate rejects OS-only rendering, stale accent, custom drawing and obscured controls", () => {
  for (const mutate of [f => { f.style.renderer = "auto"; }, f => { f.style.accentColor = "rgb(0, 120, 255)"; },
