-- Test-only migration driver. The native wizard has already opened NSOpenPanel;
-- exact OS attribution scopes every keystroke to the isolated browser process.
...
on run arguments
  ...
  -- The actual native migration control opened the picker. Never send a
  -- second activation key or replace its selection with a synthetic file.
  my requireFrontmost(ownedPID)
