...
All 687 tests, every combined native gate, actual anonymous update discovery
and real DMG packaging passed. The public DMG's bytes and checksum were
independently verified against staging. See
[release provenance](provenance/v0.60.0-preview.1.md).
...
- preview pointer movement without writing preferences until release, and
  cancel with Escape or when the gesture loses ownership;
- resize with arrow keys, Home and End, including right-to-left interfaces;
...
- recompute the retained row from Gecko's current selection whenever Gecko
  continues to report the group as collapsed, without copying group state;
...
- add pure projection coverage for expanded, collapsed-active,
  collapsed-inactive, changed-selection, and malformed states;
- add a packaged macOS gate that creates a real three-page Gecko group,
  ...
  Down into the next visible page, and verifies the group returns to one
  concise heading row.
