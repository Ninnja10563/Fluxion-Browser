  ...
  async function run() {
    ...
    if (phase === "seed") {
      ...
      try {
        ...
        assert(rows.length === 2, "Seed must contain both native vectors");
        write("safeBytes", rows[1].getResultByName("bytes"));
        assert((await db.execute("SELECT count(*) AS n FROM pages"))[0].getResultByName("n") === 2, "Seed evidence missing");
      ...
      report.checks.push("disabled-startup-removes-sensitive-page-and-vector", "safe-evidence-and-vector-bytes-preserved", "ordinary-places-visits-retained");
    }
    disabled();
